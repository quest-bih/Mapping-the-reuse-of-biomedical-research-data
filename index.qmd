...
![Flowchart detailing the detection of Charité datasets mentioned in the DCC. The flowchart shows the process from obtaining open and restricted-access dataset identifiers using both ODDPub and DataStet sources, through identification of datasets mentioned in the DCC, up to the overall number of instances of dataset reuse. Data identified using ODDPub include datasets added after using ODDPub-identified datasets as a seed.](plots_files/plots_png/flowchart_high_res/flowchart.svg){#fig-dist-counts width="100%"}
:::

::: {.content-visible when-format="pdf"}
![Flowchart detailing the detection of Charité datasets mentioned in the DCC. The flowchart shows the process from obtaining open and restricted-access dataset identifiers using both ODDPub and DataStet sources, through identification of datasets mentioned in the DCC, up to the overall number of instances of dataset reuse. Data identified using ODDPub include datasets added after using ODDPub-identified datasets as a seed.](plots_files/plots_png/flowchart_high_res/flowchart.pdf){#fig-dist-counts width="\\linewidth"}
:::
...
::: {#fig-dcc-side-by-side layout-ncol="2" fig-cap="(a) Frequency Distribution of Datasets Mentions (N=175). The histogram displays the number of times individual datasets were mentioned across all mentioning articles. The x-axis indicates the frequency of mentions per dataset, while the y-axis shows the number of datasets corresponding to each mention frequency. (b) Contribution of each data source to identified mentioned datasets (N = 175). The Venn diagram shows the number of mentioned datasets that originated from each source (ODDPub and DataStet), including the overlap between the sources. ODDPub includes datasets that were added using ODDPub-identified datasets as a seed."}
::: {style="height: 420px;"}
...
A reproducibility package which includes data, code, and text is deposited in ... \[Github\]. The release which corresponds to the version used to generate the article was also deposited at ... \[Zenodo\]. New versions of the article will be accompanied by new releases in Github and new versions of the deposit in Zenodo.
...
Distribution of Mentions Across Repositories (N = 1,497). The bar plot shows the frequency of dataset mentions by repository across all mentioning articles. Individual datasets may be counted multiple times per repository if they were mentioned more than once. The figure is based on 175 mentioned datasets.
:::
...
Distribution of Mentions by dataset age. The plot shows the relation between the age of datasets and the number of times it was mentioned. The dataset age (X axis) is the number of years that have passed between the dataset publication year and the publication year of the mentioning article. Dataset identifiers are added in blue text, and the number of mentions during a particular age bin is added in red text. The figure includes all reused datasets (N = 175), many of which are used only once.
:::
