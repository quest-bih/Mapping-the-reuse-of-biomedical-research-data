...
lastest_metadata_file <- load_latest_metadata_update() 

# get data
df <- lastest_metadata_file |>
  dplyr::filter(
    !is.na(license)
...
# convert to logical
df_cleaned <- df |>
  mutate(across(c(in_dcc,
                  # data_availability_statement,
                  license,
                  human_data,
                  covid_related), ~ .x == "TRUE"))
# 
...
p_raw <- c(#p_das,
  ...
  p_human,
  p_covid)
names(p_raw) <- c(#"data_availability_statement",
  "license", "human_data", "covid_related")
...
tibble(
  variable = names(p_raw),
  `p value` = sprintf("%.3f", p_fdr)
)

# Output all p-values (formatted nicely)
tibble(
  variable = names(p_raw),
  p_raw        = sprintf("%.3f", p_raw),
  p_bonferroni = sprintf("%.3f", p_bonferroni),
