...
geom_text(
  data = year_dcc_dt_all_for_plot_agg |>
    group_by(year_dcc) |>
    dplyr::filter(!(year_dcc %in% c(2012, 2013, 2015, 2017) & count == min(count))),  # Exclude only in specific years
...
ggplot(random_data, aes(x = x, y = y)) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +  # Solid fill (no gradient)
