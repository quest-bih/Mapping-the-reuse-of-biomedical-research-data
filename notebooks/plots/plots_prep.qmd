...
load(here("data", "wrangling_steps", "all_sources_binded",                                                      
          "dcc_detected_ids_all_sources_6_src_fix.RData"))

# create a table with no overlapping sources
dcc_charite_joined_final_no_source_overlap <- dcc_detected_ids_all_sources_6_src_fix |> 
  dplyr::filter(
    # exclude datastet+additional that are already in numbat
    source_charite %in% c(
      "data_articles", # there is one that is "in numbat as well", but with a different ID, so no numbat match with it
      "numbat",
      "additional_ids",
      "datastet")) |> 
    mutate(
...
dcc_charite_joined_final_dup_sources <- dcc_detected_ids_all_sources_6_src_fix


rm(dcc_detected_ids_all_sources_6_src_fix)
```
...
dcc_summary <- dcc_charite_joined_final_no_source_overlap |>
...
# counts_final <- tibble(
#   `Mentioning Articles` = dcc_charite_joined_final_no_source_overlap |> select(doi_dcc) |> distinct() |> nrow(),
#   `Charité Articles` = dcc_charite_joined_final_no_source_overlap |> select(doi_charite) |> distinct() |> nrow(),
#   `Mentioned Datasets` = dcc_charite_joined_final_no_source_overlap |> select(detected_id) |> distinct() |> nrow()
# )
# 
# # save as RData
...
ds_freq <- dcc_charite_joined_final_no_source_overlap |>
...
rep_dt <- dcc_charite_joined_final_no_source_overlap |> 
...
year_dcc_dt_all <- dcc_charite_joined_final_no_source_overlap |>
  select(doi_dcc, detected_id, publication_year_dcc) |> 
  distinct() |> 
  count(detected_id, sort = TRUE)

year_dcc_dt_all_for_plot <- year_dcc_dt_all |>
  inner_join(dcc_charite_joined_final_no_source_overlap, by = "detected_id")
...
dcc_charite_for_sankey <- dcc_charite_joined_final_no_source_overlap |>
  ...
  mutate(repository = case_when(repository %in% c("elsevier data repository", "elsevier bv", "mendeley")
                                ~ "Mendeley", .default = repository),
         covid_related = case_when(detected_id == "mt108784"
                                   ~ "TRUE", .default = covid_related),
         repository = case_when(detected_id == "ng00075"
                                ~ "european nucleotide archive", .default = repository)) |> 
  distinct()
...
id_source_for_venn <- dcc_charite_joined_final_dup_sources |>
  bind_rows(
    dcc_charite_joined_final_dup_sources |> 
      slice_head(n = 2) |> 
