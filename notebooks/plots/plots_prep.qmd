...
rep_dt <- dcc_charite_joined_final |> 
  ...
  mutate(repository = case_when(
    ...
    repository == "elsevier data repository" ~ "elsevier",
...
dif_for_plot <- dcc_charite_joined_final |>
  select(data_id_merged, publication_year_dcc, charite_id_year) |>
  distinct() |>
  mutate(dif = publication_year_dcc - charite_id_year) |>
  dplyr::filter(dif < 24 | dif > 0) # removing "dbgap" datasets
