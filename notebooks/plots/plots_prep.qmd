...
# 
# dcc_charite_for_sankey <- datasets_metadata_master_updated_003 |>
...
So I could plot 1/2/3 years difference on x axis, and then how many mentions for each dataset.
Try to normalize so that we could really see the change throughout the years
...
ds_age_cit_cor_prep |> 
  ...
  labs(
    ...
    x = "Dataset Age When Cited (Years)",
...
Create a table like in the reference pic that I took, with:
Dimensions of metadata on one side (human, covid, etc)
3 column with colors for the p value of x sqr: matched, non matched, p value
...
not_mentioned_years <- datasets_metadata_master_updated_003 |> 
