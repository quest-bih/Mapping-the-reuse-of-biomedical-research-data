...
# 2. DOIs, IDs and Mentions - Only Data Articles
...
counts_ids_and_dois_da <- tibble(
  ...
  count = c(
    charite_da_dois_and_ids |>
      select(charite_doi) |>
      distinct() |>
      nrow(), # number of charite dois with data articles
    
    charite_da_dois_and_ids |>
      select(identifier_from_data_article_analysis) |>
      distinct() |>
      nrow(), # number of data articles
    
    charite_da_dois_and_ids |>
      select(identifier_from_data_article_analysis) |>
      distinct() |>
      nrow(), # number of data articles datasets ### FIX
    
    data_articles_dois_v10 |>
      distinct() |> 
      nrow(), # number of mentions
    
    data_articles_dois_v10 |>
      distinct() |> 
      nrow() # number of mentions (extrapolation) ### FIX
  )
)
