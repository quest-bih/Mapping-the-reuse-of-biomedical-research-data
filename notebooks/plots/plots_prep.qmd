...
dcc_charite_joined_final <- dcc_detected_ids_numbat_w_metadata |> 
  dplyr::filter(data_id_source == "numbat")
rm(dcc_detected_ids_numbat_w_metadata)
```
...
counts_final <- tibble(
  ...
  `Mentioned Datasets` = dcc_charite_joined_final |> select(detected_in_dcc) |> distinct() |> nrow()
)
...
counts_w_au_ov <- tibble(
  ...
  `Mentioned Datasets` = dcc_charite_joined_w_au_ov |> select(detected_id) |> distinct() |> nrow()
)
...
rep_dt <- dcc_charite_joined_final |> 
  ...
  select(repository, detected_in_dcc, doi_dcc) |> 
  distinct() |> 
  group_by(repository) |> 
  summarise(n = n()) |> 
  arrange(desc(n))
...
year_dcc_dt_all <- dcc_charite_joined_final |>
  select(doi_dcc, detected_in_dcc, publication_year_dcc) |> 
  distinct() |> 
  count(detected_in_dcc, sort = TRUE)

year_dcc_dt_all_for_plot <- year_dcc_dt_all |>
  inner_join(dcc_charite_joined_final, by = "detected_in_dcc")

year_dcc_dt_all_for_plot_agg <- year_dcc_dt_all_for_plot |> 
  mutate(
    detected_in_dcc = case_when(
      !detected_in_dcc %in% c("6y2e", "gse14764", "gse148729", "gse55235") ~ "Others",
      .default = detected_in_dcc),
  ...
  group_by(detected_in_dcc, publication_year_dcc) |> 
...
dcc_charite_for_sankey <- dcc_charite_joined_final |>
  select(detected_in_dcc, orig_id_is_doi, license_for_analysis, data_access, covid_related, repository) |>
  distinct()
...
ds_age_cit_cor_prep <- dcc_charite_joined_final |>
  select(detected_in_dcc, doi_dcc, publication_year_dcc, charite_id_year) |>
  dplyr::filter(charite_id_year > 2019) |>
  mutate(ds_age_when_cited = publication_year_dcc - charite_id_year) |>
  count(ds_age_when_cited, detected_in_dcc, name = "n_citations") |>
  dplyr::filter(ds_age_when_cited >= 0)

# get only datasets that are cited at every age:
...
# tag datasets that appear more than once (for line connection)
ds_tagged <- ds_age_cit_cor_prep |>
  dplyr::group_by(detected_in_dcc) |>
  dplyr::mutate(n_instances = dplyr::n()) |>
  dplyr::ungroup() |>
  dplyr::mutate(has_line = n_instances > 1)

# save as RData
save_cr(ds_age_cit_cor_prep, file = file.path(here("data",
                                         "tables_for_plots",
                                         "ds_age_cit_cor_prep.RData")))

# save as RData
save_cr(ds_tagged, file = file.path(here("data",
                                         "tables_for_plots",
                                         "ds_tagged.RData")))
...
#   select(charite_id_year, detected_in_dcc) |> 
#   distinct() |> 
...
```{r}
id_source_for_venn <- dcc_detected_ids_numbat_w_metadata |> 
  select(detected_in_dcc, data_id_source)

# venn_input_for_debugging <- id_source_for_venn |>
#   distinct(detected_in_dcc, data_id_source) |>  # remove duplicate rows
#   pivot_wider(names_from = data_id_source, values_from = data_id_source,
#               values_fn = length, values_fill = 0) |>
#   mutate(across(-detected_in_dcc, ~ .x > 0)) # create a TRUE/FALSE matrix for debugging
