...
data_articles_dois <- read_excel(file.path(here("data",
                                                "raw",
                                                "data_articles"), "datajournal_articles - analysis of citations v7.xlsx"),
...
# for openalex i need 2 csvs as outputs - 1 for each doi column (into "data\results_verification\meta data for data articles dois")
...
Since the openalex script couldn't handle most of the added DOIs,
and since there were only a few of them, I:
1. Loaded previously added metadata
2. Left joined it with the current DOIs list
...
Commented out chunks below were assuming that OpenAlex would work.
...
# original list with both charite and reusing DOIs
data_articles_dois <- data_articles_dois |> 
    mutate(reusing_paper_doi = str_remove(reusing_paper_doi, "^https://doi.org/"))

# Join the existing metadata into the new DOIs list
...
data_articles_dois_1_with_metadata <- data_articles_dois |> 
  ...
  left_join(data_articles_reusing_metadata, by = "reusing_paper_doi", suffix = c("", "_reusing")) 
...
# save
save_cr(data_articles_dois_3_au_ov_info,
        file = file.path(here("data",
                                "wrangling_steps",
                                "data_articles",
